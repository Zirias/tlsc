...
static FILE *openpidfile(const char *pidfile)
{
	...
	{
	    Log_fmt(L_ERROR, "error getting lock info on `%s'", pidfile);
	    return 0;
	}
	...
	if (!locked)
	{
	    ...
	    {
		Log_fmt(L_WARNING, "removing stale pidfile `%s'", pidfile);
		if (unlink(pidfile) < 0)
		{
		    Log_fmt(L_ERROR, "cannot remove `%s'", pidfile);
		    return 0;
		}
		fclose(pf);
	    }
	...
	if (locked)
	{
	    ...
	    {
		if (prc < 1) pid = -1;
		Log_fmt(L_ERROR, "pidfile `%s' content (pid %d) and lock "
			"owner (pid %d) disagree! This should never "
			"happen, giving up!", pidfile, pid, pflock.l_pid);
	    }
	    else
	    {
		Log_fmt(L_ERROR, "daemon already running with pid %d", pid);
	    }
	    return 0;
	}
    }
    ...
    if (!pf)
    {
	Log_fmt(L_ERROR, "cannot open pidfile `%s' for writing", pidfile);
	return 0;
    }
    memset(&pflock, 0, sizeof pflock);
    ...
    {
	fclose(pf);
	Log_fmt(L_ERROR, "locking own pidfile `%s' failed", pidfile);
	return 0;
    }
	...
	const char *pidfile, int waitLaunched)
{
	...
	Log_msg(L_ERROR, "failed to fork (1)");
	goto done;
    }
    ...
    if (pid < 0)
    {
	Log_msg(L_ERROR, "failed to fork (2)");
	goto done;
    }
    ...
    Log_msg(L_INFO, "forked into background");
    rc = dmain(data);
